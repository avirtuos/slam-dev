...
private:
    ...
    bool isDuplicate(TelemetryPoint result_buffer[], const int result_size, int16_t x, int16_t y);
    bool is_connected;
    const char *com_port;
    _u32 baudrate;
    RPlidarDriver *drv;
...
bool LaserScan::isDuplicate(TelemetryPoint result_buffer[], const int result_size, int16_t x, int16_t y){
    for(int i = 0; i < result_size; i++){
        if(result_buffer[i].x == x && result_buffer[i].y == y){
            return true;
        }
    }
    return false;
